...
function getMapZone(x: number, y: number, playerTeamId: number): { zone: MapZone; safety: 'safe' | 'neutral' | 'danger' } {
  // Summoner's Rift coordinates roughly:
  // Blue base: (0-3000, 0-3000)
  // Red base: (12000-15000, 12000-15000)
  // River: diagonal from (4000, 11000) to (11000, 4000)
...
// Get game phase based on timestamp
...
// Check if position is under enemy tower
function isUnderEnemyTower(position: { x: number; y: number }, playerTeamId: number): boolean {
  // Tower positions for both teams (approximate)
  const blueTowers = [
    { x: 1512, y: 1336 }, // Nexus
    { x: 1169, y: 4287 }, // Top inner
    { x: 4318, y: 1029 }, // Bot inner
    { x: 981, y: 10441 }, // Top outer
    { x: 6919, y: 1483 }, // Bot outer
  ];

  const redTowers = [
    { x: 13604, y: 13350 }, // Nexus
    { x: 13866, y: 10648 }, // Top inner
    { x: 10504, y: 13604 }, // Bot inner
    { x: 8955, y: 13607 }, // Top outer
    { x: 14340, y: 8012 }, // Bot outer
  ];
...
export function analyzeDeaths(
  ...
  for (const frame of frames) {
    for (const event of frame.events) {
      ...
      const { zone, safety } = getMapZone(position.x, position.y, playerTeamId);
      ...
      // Determine death type and severity
      ...
      // Analyze different death scenarios
      if (wasUnderTower) {
        ...
        coachingNote = safety === 'danger'
          ? `Tu etais en territoire ennemi (${zone.replace('_', ' ')}). C'est tres risque sans ton equipe.`
